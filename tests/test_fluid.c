...
void test_backward_linear_basic(void) {
    ...
    // Expected gradients
    ASSERT_NEAR(-0.5f, bf16_to_float(t_grad_w.data[0]), BF16_TOLERANCE);
    ASSERT_NEAR(-1.0f, bf16_to_float(t_grad_w.data[1]), BF16_TOLERANCE);
...
void test_optimizer_sgd_basic(void) {
    ...
    ASSERT_NEAR(0.55f, bf16_to_float(t_w.data[0]), BF16_TOLERANCE);
    ASSERT_NEAR(0.60f, bf16_to_float(t_w.data[1]), BF16_TOLERANCE);
...
void test_sgd_zero_gradient(void) {
    ...
    ASSERT_NEAR(1.0f, bf16_to_float(t_w.data[0]), BF16_TOLERANCE);
    ASSERT_NEAR(2.0f, bf16_to_float(t_w.data[1]), BF16_TOLERANCE);
...
void test_sgd_large_lr(void) {
    ...
    // W = W - 1.0 * grad
    ASSERT_NEAR(0.0f, bf16_to_float(t_w.data[0]), BF16_TOLERANCE);  // 1 - 1 = 0
    ASSERT_NEAR(2.0f, bf16_to_float(t_w.data[1]), BF16_TOLERANCE);  // 1 - (-1) = 2
...
void test_sgd_convergence(void) {
    ...
    for (int step = 0; step < 20; step++) {
        float w = bf16_to_float(t_w.data[0]);
        float grad = w - target;  // dL/dw
        t_grad.data[0] = float_to_bf16(grad);
        
        optimizer_sgd(&fp, lr);
    }
    
    float final_w = bf16_to_float(t_w.data[0]);
...
void test_backward_linear_larger(void) {
    ...
    ASSERT_NEAR(1.0f, bf16_to_float(t_grad_w.data[0]), BF16_TOLERANCE);
    ASSERT_NEAR(4.0f, bf16_to_float(t_grad_w.data[1]), BF16_TOLERANCE);
    ASSERT_NEAR(2.0f, bf16_to_float(t_grad_w.data[2]), BF16_TOLERANCE);
    ASSERT_NEAR(5.0f, bf16_to_float(t_grad_w.data[3]), BF16_TOLERANCE);
    ASSERT_NEAR(3.0f, bf16_to_float(t_grad_w.data[4]), BF16_TOLERANCE);
    ASSERT_NEAR(6.0f, bf16_to_float(t_grad_w.data[5]), BF16_TOLERANCE);
