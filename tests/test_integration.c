...
void test_arena_tensor_integration(void) {
    ...
    // Write and read back
    for (int i = 0; i < 32; i++) {
        t.data[i] = float_to_bf16((float)i);
    }
    
    ASSERT_NEAR(0.0f, bf16_to_float(t.data[0]), BF16_TOLERANCE);
    ASSERT_NEAR(15.0f, bf16_to_float(t.data[15]), BF16_TOLERANCE);
    ASSERT_NEAR(31.0f, bf16_to_float(t.data[31]), BF16_TOLERANCE);
...
void test_forward_pass_simple(void) {
    ...
    float sum_sq = 0.0f;
    for (int i = 0; i < dim; i++) {
        float val = bf16_to_float(normed.data[i]);
        sum_sq += val * val;
    }
...
void test_kv_cache_eviction_pipeline(void) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        ASSERT_NEAR(expected_k0, bf16_to_float(cache.k.data[i * 2]), BF16_TOLERANCE_LOOSE);
    }
...
void test_sparse_attention_flow(void) {
    ...
    // Verify scores
    ASSERT_NEAR(0.1f, bf16_to_float(t_scores.data[0]), BF16_TOLERANCE);
    ASSERT_NEAR(0.8f, bf16_to_float(t_scores.data[1]), BF16_TOLERANCE);
    ASSERT_NEAR(0.3f, bf16_to_float(t_scores.data[2]), BF16_TOLERANCE);
    ASSERT_NEAR(0.9f, bf16_to_float(t_scores.data[3]), BF16_TOLERANCE);
    ASSERT_NEAR(0.5f, bf16_to_float(t_scores.data[4]), BF16_TOLERANCE);
...
void test_nested_learning_loop(void) {
    ...
    t_x.data[0] = float_to_bf16(1.0f);
    
    t_tensor t_w = tensor_view(w_buf, shape, 2);
    t_w.data[0] = float_to_bf16(0.0f);  // Start at 0
    ...
    for (int step = 0; step < 10; step++) {
        // Forward: y = x * w
        float x = bf16_to_float(t_x.data[0]);
        ...
        t_grad.data[0] = float_to_bf16(grad_w);
...
void test_sampler_integration(void) {
    ...
    for (int i = 0; i < 100; i++) {
        logits.data[i] = float_to_bf16(-1.0f);
    }
    
    // Set token 42 as winner
    logits.data[42] = float_to_bf16(10.0f);
